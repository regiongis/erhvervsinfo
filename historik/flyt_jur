CREATE OR REPLACE FUNCTION cvr.flyt_jur(komkode integer, startdato date, slutdato date)
  RETURNS TABLE(
    status text,
    "cvr-nummer" bigint,
...
FROM (
       SELECT 'Tilflytter'::text AS flyttemoenster,
          ...
          LEFT JOIN (
                       SELECT
                          ...
                          belig_adresse_postdistrikt,
						              geom,
                          indlaest_dato
                       FROM cvr.cvr_jur_enhed_geo_hist_adresser c
                       WHERE beliggenhedsadresse_gyldigfra = (
                         SELECT MAX(beliggenhedsadresse_gyldigfra)
                         FROM cvr.cvr_jur_enhed_geo_hist_adresser d
                         WHERE c.cvrnr = d.cvrnr AND d.beliggenhedsadresse_gyldigfra < startdato
                       )
                     ) AS b ON a.cvrnr = b.cvrnr
        ...
        SELECT 'Nystartet'::text AS flyttemoenster,
          ...
          a.geom,
          a.indlaest_dato
        FROM cvr.cvr_jur_enhed_geo_stam a
...
JOIN cvr.cvr_jur_enhed_geo_stam stam ON flyt.cvrnr = stam.cvrnr
